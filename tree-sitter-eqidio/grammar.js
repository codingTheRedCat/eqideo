...
module.exports = grammar({
  ...
  word: $ => $.command_name,
  rules: {
    source_file: $ => repeat($._eqidio_definition),

      _eqidio_definition: $ => choice($.eqidio_latex, $.eqidio_command),
      eqidio_latex: $ => prec.left(seq("$$", optional($._eqidio_latex_content), "$$")),
      eqidio_command: $ => prec.right(seq(field("command",$.eqidio_command_identifier), field("argument", repeat(choice($.eqidio_identifier, $.eqidio_int, $.eqidio_float))))), 
      eqidio_command_identifier: $ => /[a-zA-Z0-9_]+;/,
      eqidio_identifier: $ => /[a-zA-Z_][a-zA-z0-9_]*/,
      eqidio_int: $ => /[+-]?[0-9]+/,
      eqidio_float: $ => /[-+]?[0-9]*\.?[0-9]+([eE][-+]?[0-9]+)?/,
    _eqidio_latex_content: $ => repeat1($._root_content),
